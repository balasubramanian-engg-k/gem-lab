...
<div class="max-w-7xl mx-auto p-6">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Invoice #AD{{ str_pad($invoice->id, 6, '0', STR_PAD_LEFT) }}</h1>
        ...
        <form action="{{ route('invoices.update', $invoice->id) }}" method="POST" id="invoiceForm">
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location', $invoice->location ?? '') }}" required
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="total_count" min="0" value="{{ old('total_count', $invoice->total_count ?? '') }}" required
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Actual Silver Weight <span class="text-red-500">*</span></label>
                        <input type="number" name="actual_silver_weight" step="0.01" min="0" value="{{ old('actual_silver_weight', $invoice->actual_silver_weight ?? '0.00') }}" required
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="wastage_making_certification_cost" step="0.01" min="0" value="{{ old('wastage_making_certification_cost', $invoice->wastage_making_certification_cost ?? '0.00') }}" required
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        ...
                        <select name="product_type_id" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @foreach($productTypes as $productType)
                                <option value="{{ $productType->id }}" {{ old('product_type_id', $invoice->product_type_id) == $productType->id ? 'selected' : '' }}>{{ $productType->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @foreach(\App\Models\Invoice::STATUSES as $s)
                                <option value="{{ $s }}" {{ (old('status', $invoice->status ?? 'NEW') == $s) ? 'selected' : '' }}>{{ $s }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Assignee Name <span class="text-red-500">*</span></label>
                        <input type="text" name="assignee_name" value="{{ old('assignee_name', $invoice->assignee_name ?? '') }}" required
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div class="md:col-span-2">
                        ...
                        <textarea name="remarks" rows="3" required
                                  class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">{{ old('remarks', $invoice->remarks ?? '') }}</textarea>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                        <input type="date" name="due_date" value="{{ old('due_date', $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('Y-m-d') : '') }}"
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Silver Rate</label>
                        <input type="number" name="silver_rate" step="0.01" min="0" value="{{ old('silver_rate', $invoice->silver_rate !== null ? number_format((float) $invoice->silver_rate, 2, '.', '') : '0.00') }}"
                               class="w-full border border-gray-300 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="mt-6 flex justify-end space-x-3">
                ...
                <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Update Invoice</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const invoiceDetailsRaw = @json($invoice->invoiceDetails ?? []);
    // Ensure invoiceDetails is an array
    const invoiceDetails = Array.isArray(invoiceDetailsRaw) ? invoiceDetailsRaw : Object.values(invoiceDetailsRaw);

    // Debug: Log invoice details
    console.log('Invoice Details:', invoiceDetails);
    console.log('Invoice Details Count:', invoiceDetails.length);
    ...
    function addProductRow(detail = null) {
        ...
        const stoneId = detail ? (detail.stone || '') : '';
        const ringSize = detail ? (detail.ring_size || '') : '';
        const stoneWeight = detail ? (detail.ston_weight || '') : '';
        const grossWeight = detail ? (detail.gross_weight || '') : '';

        // Build stone options with proper selected attribute
        const stoneOptions = stones.map(stone => {
            // Convert both to strings for comparison to handle type mismatches
            const isSelected = String(stone.id) === String(stoneId) ? 'selected' : '';
            return `<option value="${stone.id}" data-rate="${stone.rate_per_piece}" ${isSelected}>${stone.stone_name}</option>`;
        }).join('');
        ...
        row.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="products[${productCount}][stone_weight]" step="0.01" min="0" required
                           value="${stoneWeight || '0.00'}"
                ...
                <div>
                    ...
                    <input type="number" name="products[${productCount}][gross_weight]" step="0.01" min="0" required
                           value="${grossWeight || '0.00'}"
    ...
    if (invoiceDetails && invoiceDetails.length > 0) {
        ...
        // Add first product row if no existing details
        addProductRow();
    }
