...
<div class="max-w-xl mx-auto p-6">
    <div class="bg-white rounded-lg shadow-md p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Sell silver</h1>
        <p class="text-sm text-gray-600 mb-4">Record silver sold (not used in an invoice). This deducts from your total stock. Stock can go negative if you sell more than you have.</p>

        <form method="POST" action="{{ route('stock.storeSell') }}" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <textarea name="remarks" id="remarks" rows="3" placeholder="e.g. Sold to customer X" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('remarks') }}</textarea>
                @error('remarks')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition">Save</button>
